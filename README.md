...
- Single entrypoint for day-to-day workflows: `aws-assistant ec2 ... | s3 ... | lambda ... | ecs ... | eks ...`
- Smart defaults via `~/.aws-assistant.yml` (region/profile/tag filters)
- Built-in safety confirmations for destructive operations (bucket delete, EC2 terminate)
...
- Optional interactive TUI mode (`aws-assistant tui`) with arrow-key menus and searchable long lists
- Quick Action Palette in TUI for power users (type intent directly, skip menu navigation)
...
## Release Notes
### v1.3.0 (2026-03-10)
- Added unified assistant UX in `aws_assistant.py`:
  - `lambda`, `container`, `ecs`, `eks` forwarding from one top-level CLI
  - `logs` helper commands for Lambda/ECS/EKS with quick error scanning
  - `iam summary` and `iam policy-template` helper commands
  - `safety scan` command for suspicious resource checks
  - `tui` interactive mode for EC2, logs, IAM templates, and safety scan
  - `config init/show` using `~/.aws-assistant.yml`
  - Destructive-op confirmations for `ec2 terminate` and `s3 delete-bucket`
- Added test coverage for assistant quality-of-life functions in `tests/test_aws_assistant_qol.py`
...
- `aws-assistant`
- `aws-assistant-s3`
- `aws-assistant-lambda`
- `aws-assistant-container`
